"""
Explainability demo for the trained BUSI model.

Loads BUSI_best_model.pth (EfficientNet-V2S + regression head, the ESPACOL
Hybrid Contrastive Ordinal model), picks one BUSI image, runs the explainability
pipeline (explainability.py), and pops up a SINGLE window with five panels:
...
Run (from the repo root, ESPACOL-New/):
    python Hady/demo_explain_busi.py                 # random image, biomedclip
    python Hady/demo_explain_busi.py --index 0       # deterministic single image
    python Hady/demo_explain_busi.py --encoder random          # offline, no download
    python Hady/demo_explain_busi.py --save out.png --no-show  # headless, write PNG
...
* BUSI_best_model.pth was trained WITHOUT the image-text alignment loss, so even
...
from Datasets.dataloaders import BUSIDataset, _IMAGENET_MEAN, _IMAGENET_STD
...
def _safe_mask(image_np: np.ndarray) -> np.ndarray:
    """Soft ultrasound-region mask; falls back to all-ones if scipy is missing."""
    try:
        return _ultrasound_mask(image_np)
    except Exception:
        return np.ones(image_np.shape[:2], dtype=np.float32)


def _apply_mask(heatmap: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """Suppress out-of-tissue activations, then renormalize to [0,1] (as visualize() does)."""
    hm = heatmap * mask
    if hm.max() > hm.min():
...
def build_combined_figure(plt, pipeline, image_np, result, true_name, encoder):
    ...
    fig.suptitle(
        f"ESPACOL explainability  |  BUSI  |  {result['predicted_class']}"
...
# ─────────────────────────────────────────────────────────────────────────────
# Dataset root resolution (mirrors demo_busi.py)
# ─────────────────────────────────────────────────────────────────────────────

def resolve_busi_root(explicit):
    """Return the first valid BUSI dataset root (contains benign/malignant/normal/), or None."""
    def _valid(p):
    ...
    candidates += [
        os.path.join(_REPO_ROOT, "Datasets", "BUSI"),
        os.path.join(_REPO_ROOT, os.pardir, "Datasets", "BUSI"),
    ]
    for c in candidates:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="BUSI explainability demo: pops up Original | WHERE | WHY | WHY+WHERE."
    )
    parser.add_argument(
        "--checkpoint",
...
def main():
    ...
    print(f"Result     : {'CORRECT' if result['predicted_label'] == int(y) else 'WRONG'}")
    print("-" * 60)
