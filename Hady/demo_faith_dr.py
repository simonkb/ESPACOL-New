"""
Faithfulness STEP-1 (concept-drop) demo for the trained DR (diabetic retinopathy) model.
...
Run (from the repo root, ESPACOL-New/):
    python Hady/demo_faith_dr.py                       # random image (usually grade 0)
    python Hady/demo_faith_dr.py --grade 4             # random Proliferative-DR image
    python Hady/demo_faith_dr.py --index 0             # deterministic single image
    python Hady/demo_faith_dr.py --occlusion gray      # mean/gray occlusion instead of blur
    python Hady/demo_faith_dr.py --grade 4 --save out.png --no-show   # headless PNG

Notes
-----
* Mirrors demo_explain_dr.py for model/encoder/dataset setup and reuses its helpers;
  the genuinely-new logic (occlude / re-score / drop-check / figure) lives in
  Hady/faithfulness.py.
* DR caveat: for severe/proliferative DR all concept cosines go negative — the
  concept-drop check is SIGN-AGNOSTIC (it uses c_before - c_after), so a faithful H_c
  still yields a positive drop. Read the DROP, never the absolute sign.
* The concept-drop is only interpretable with --encoder biomedclip AND the recovered
  text projection (Hady/DR_text_projection.pth). With --encoder random the WHY scores
  are placeholders and the drop is meaningless (a warning is printed).
* ~74% of DR images are grade 0 (No DR); use --grade to see a specific severity.
"""
...
from explainability import ExplainabilityPipeline, DR_CONCEPTS
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="DR faithfulness step-1 (concept-drop) demo: "
    ...
    parser.add_argument(
        ...
        help="Do not restrict occlusion to the fundus region (occlude the raw H_c).",
    )
    parser.add_argument(
...
def main():
    ...
    # Sign-agnostic: for severe/proliferative DR all cosines are negative, but a
    # faithful H_c still produces a positive drop (c_before - c_after).
    ...
    fig = build_faithfulness_figure(
        ...
        mask, true_name, pred_before, pred_after, dataset_name="DR",
