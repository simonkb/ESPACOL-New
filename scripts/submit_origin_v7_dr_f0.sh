#!/bin/bash
# ORIGIN-v7 identified sparse relation pilot: hash-bound v3 EyePACS fold 0.
# This has the same optimizer, objective, data split, and epoch budget as v6;
# only the relation compiler changes.
#SBATCH --job-name=origin_v7_dr0
...
#SBATCH --output=/dpc/kuin0170/ESPACOL-New/origin_v7_dr_f0_%j.out
#SBATCH --error=/dpc/kuin0170/ESPACOL-New/origin_v7_dr_f0_%j.err
...
RUN_DIR="${ORIGIN_RUN_DIR:-runs/origin_dr_f0_v7_identified_sparse_k8}"
DATA_ROOT="${ORIGIN_DR_ROOT:-Datasets/DR}"
SOURCE_CHECKPOINT="${ORIGIN_V3_CHECKPOINT:-runs/origin_dr_f0_v3_bounded/fold0/best.pth}"
...
if [[ "${ORIGIN_RESUME:-0}" == "1" ]]; then
  [[ -f "${FOLD_DIR}/last.pth" && ! -f "${FOLD_DIR}/result.json" ]] || {
    echo "Cannot resume missing or completed ORIGIN-v7 run: ${FOLD_DIR}." >&2
    exit 3
  }
  RESUME_ARGS+=(--resume)
elif [[ -e "${FOLD_DIR}/last.pth" || -e "${FOLD_DIR}/history.csv" ]]; then
  echo "Fresh ORIGIN-v7 run refused: artifacts exist in ${FOLD_DIR}." >&2
  exit 4
fi

echo "=== ORIGIN-v7 identified-sparse EyePACS fold 0 / inner validation only ==="
echo "controlled_change=masked_two_way_identification+fixed_top8_edge_ledger;v3_base=frozen"
...
TRAIN_ARGS=(
  ...
  --relation_variant identified_sparse_v1 --relation_edge_budget 8
  ...
  --relation_only_epochs 25
  --batch_size 8 --epochs 25 --num_workers "${ORIGIN_NUM_WORKERS:-8}"
  --lr 1e-4 --head_lr 5e-4 --weight_decay 1e-5 --freeze_encoder_epochs 2
  --scheduler plateau --lr_factor 0.2 --lr_patience 5
  --early_stopping_patience 25 --rps_weight 0.25
