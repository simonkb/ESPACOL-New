#!/bin/bash
# Matched OSWT control with the adjacent-posterior risk gate removed.
#SBATCH --job-name=oswt_u0
...
#SBATCH --output=/dpc/kuin0170/ESPACOL-New/paths_oswt_aptos_ungated_f0_%j.out
#SBATCH --error=/dpc/kuin0170/ESPACOL-New/paths_oswt_aptos_ungated_f0_%j.err
...
echo "=== PATHS-V8 ungated shell control APTOS fold 0 / inner validation only ==="
...
PATHS_OSWT_RUN_GIT_COMMIT="$(git rev-parse HEAD)" python train_paths_oswt.py \
  ...
  --run_dir "${PATHS_OSWT_APTOS_UNGATED_RUN_DIR:-runs/paths_oswt_aptos_f0_v8_ungated}" \
  --folds 0 --seed 42 \
  --v3_checkpoint "${V3}" --v3_sha256 "${PATHS_OSWT_APTOS_V3_SHA256}" \
  --oswt_variant shell_warranted_ungated \
