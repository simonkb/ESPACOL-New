#!/bin/bash
# EyePACS fold 0. Launch only through the prospective APTOS promotion gate.
#SBATCH --job-name=oswt_dr0
...
#SBATCH --output=/dpc/kuin0170/ESPACOL-New/paths_oswt_dr_f0_%j.out
#SBATCH --error=/dpc/kuin0170/ESPACOL-New/paths_oswt_dr_f0_%j.err
...
GATE="${PATHS_OSWT_APTOS_RUN_DIR:-runs/paths_oswt_aptos_f0_v8_gated}/fold0/PROMOTED_TO_EYEPACS.json"
[[ -f "${GATE}" ]] || { echo "Prospective APTOS promotion gate is absent." >&2; exit 2; }
python - "${GATE}" "$(git rev-parse HEAD)" <<'PY'
...
if checksum != observed:
    raise SystemExit("APTOS OSWT gate checksum mismatch")
if payload.get("schema") != "paths-v8-oswt-complementary-aptos-paired-gate-v2" or not payload.get("passed"):
    raise SystemExit("APTOS OSWT gate did not pass")
if payload.get("git_commit") != sys.argv[2]:
    raise SystemExit("APTOS OSWT gate was produced by a different Git commit")
if payload.get("implementation_signature") != paths_oswt_implementation_signature():
    raise SystemExit("APTOS OSWT gate was produced by different implementation bytes")
PY
[[ "$(sha256sum "${V3}" | awk '{print $1}')" == "${PATHS_OSWT_DR_V3_SHA256}" ]] || {
  echo "EyePACS V3 SHA-256 mismatch." >&2; exit 2;
}

echo "=== PATHS-V8 risk-gated OSWT EyePACS fold 0 / inner validation only ==="
...
PATHS_OSWT_RUN_GIT_COMMIT="$(git rev-parse HEAD)" python train_paths_oswt.py \
  ...
  --run_dir "${PATHS_OSWT_DR_RUN_DIR:-runs/paths_oswt_dr_f0_v8_gated}" \
  --folds 0 --seed 42 \
