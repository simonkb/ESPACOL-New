#!/bin/bash
# Controlled MOSAIC-v4 experiment: RF-medium + fixed 8x8 hard-existential
# regional events. APTOS fold 0, inner validation only; outer test remains locked.
#SBATCH --job-name=mosaic_a_f0_max8
...
#SBATCH --output=/dpc/kuin0170/ESPACOL-New/mosaic_aptos_max8_f0_%j.out
#SBATCH --error=/dpc/kuin0170/ESPACOL-New/mosaic_aptos_max8_f0_%j.err
...
RUN_DIR="${MOSAIC_RUN_DIR:-runs/mosaic_aptos_f0_hardmax_v4}"
FOLD=0
TOTAL_EPOCHS=35
...
if [[ -e "${FOLD_DIR}/last.pth" || -e "${FOLD_DIR}/history.csv" ]]; then
  echo "Fresh hard-existential run refused: ${FOLD_DIR} contains training artifacts." >&2
  echo "Set MOSAIC_RUN_DIR to a new directory; do not resume an incompatible checkpoint." >&2
  exit 3
fi

echo "=== MOSAIC-v4 hard-existential APTOS fold 0 / 35 epochs (inner validation only) ==="
...
# Structural proof, hard-existential compiler, ordinal nesting, integration,
# intervention, and certificate tests must pass before allocating training.
python -m pytest -q \
...
# verifies the 112x112 source -> 8x8 proof geometry, compiler identity, and a
# CUDA backward before allocating the full training budget.
python - <<'PY'
import torch
...
if model.region_pool_type != "existential_max":
    raise RuntimeError("hard-existential compiler contract failed")
if output.evidence.regional_pool_type != "existential_max":
    raise RuntimeError("hard-existential output metadata contract failed")
if model.region_pool_temperature is not None:
    raise RuntimeError("hard-existential compiler must not retain a temperature")
if output.evidence.regional_pool_temperature is not None:
    raise RuntimeError("hard-existential output must not report a temperature")
