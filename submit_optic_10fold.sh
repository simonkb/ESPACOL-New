...
# Full OPTIC v6 10-fold cross-validation on DR.
# Run submit_optic_fold0.sh first to validate >= 83% before launching this.
# v6 fixes: lr_patience=15, early_stop_patience=55, epochs=155 — see fold0 script.
python train_dr.py \
    --dr_root Datasets/DR \
    --run_dir runs/optic_dr_10fold_v6 \
