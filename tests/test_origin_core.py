...
def test_binary_chain_matches_exact_analytic_noisy_or_law() -> None:
    ...
    decoded = decode_pure_birth_rates(rates, force_fp32=False)
...
def test_rate_increase_cannot_decrease_expected_or_median_grade() -> None:
    ...
    baseline = decode_pure_birth_rates(base, force_fp32=False)
    for boundary in range(base.shape[-1]):
        increased = base.clone()
        increased[:, boundary] += 3.0
        changed = decode_pure_birth_rates(increased, force_fp32=False)
...
def test_map_is_diagnostic_not_a_monotone_decision_rule() -> None:
    ...
    before = decode_pure_birth_rates(base, force_fp32=False)
    after = decode_pure_birth_rates(increased, force_fp32=False)
