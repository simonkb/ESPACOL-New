...
from training.cross_val import DRCrossValidator, APTOSCrossValidator
...
def main():
    parser = argparse.ArgumentParser(description="Train DR/APTOS CV")
    ...
    parser.add_argument(
        ...
        default="dr",
        choices=["dr", "aptos"],
        help="Dataset: 'dr' (Kaggle DR 10-fold) or 'aptos' (APTOS 2019 5-fold)",
    )
    parser.add_argument(
        "--dr_root",
        type=str,
        default="Datasets/DR",
        help="Path to DR or APTOS dataset root",
    )
    parser.add_argument(
        "--train_csv",
        type=str,
    ...
    log.info("=" * 70)
    log.info(f"{args.dataset.upper()} CV  (EfficientNet-V2S + OPTIC-C)")
    log.info("=" * 70)
    log.info(f"Config: {cfg}")

    if args.dataset == "aptos":
        all_items = load_all_aptos_items(args.dr_root)
        cfg.n_folds = 5
        log.info(f"Total APTOS images: {len(all_items)}")
    ...
    if not args.no_cache:
        ...
        if args.dataset == "aptos":
            # APTOS has 3,662 images — RAM cache only, no disk persistence needed
            cache_dir = None
