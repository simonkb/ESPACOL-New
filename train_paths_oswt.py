#!/usr/bin/env python3
"""Train PATHS Ordinal Shell-Warranted Transport under locked splits."""
...
from configs.paths_oswt_config import PATHS_OSWT_VARIANTS, PathsOSWTConfig
...
from models.paths_oswt import build_paths_oswt_model
...
from training.paths_oswt_trainer import PathsOSWTTrainer
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="PATHS Ordinal Shell-Warranted Transport training",
    ...
    base = parser.add_argument_group("immutable audited V3 base")
    ...
    base.add_argument(
        "--scales",
        type=parse_scales,
        default=parse_scales("s4,s8,s16,s32"),
    )
    ...
    oswt = parser.add_argument_group("OSWT architecture")
    oswt.add_argument(
        "--oswt_variant",
        choices=PATHS_OSWT_VARIANTS,
        default="shell_warranted",
    )
    oswt.add_argument(
        "--pgf_probes",
        type=_parse_probes,
        default=_parse_probes("0.05,0.20,0.50,0.80"),
    )
    oswt.add_argument("--oswt_beta_init", type=float, default=0.1)
    oswt.add_argument("--oswt_tau_init", type=float, default=0.05)
    oswt.add_argument("--oswt_beta_cap", type=float, default=3.0)
    oswt.add_argument("--oswt_tau_floor", type=float, default=1e-3)
    oswt.add_argument("--oswt_tau_cap", type=float, default=0.5)
    oswt.add_argument("--oswt_strength", type=float, default=1.0)
    oswt.add_argument("--risk_set_alpha", type=float, default=0.5)
    oswt.add_argument("--rps_weight", type=float, default=0.25)

    training = parser.add_argument_group("refiner-only training")
    ...
    training.add_argument("--weight_decay", type=float, default=0.0)
    ...
    training.add_argument("--certificate_shortlist_size", type=int, default=8)
    return parser
...
def _setup_logging(run_dir: str) -> logging.Logger:
    ...
    return logging.getLogger("train_paths_oswt")
...
def require_one_sha_bound_fold(folds: list[int]) -> int:
    """Fail closed when one checkpoint/hash pair could cover multiple folds."""

    if len(folds) != 1:
        raise ValueError(
            "PATHS-OSWT accepts exactly one fold per invocation because each "
            "fold must be bound to its own audited V3 checkpoint and SHA-256; "
            "launch cross-validation as a job array with fold-specific sources"
        )
    return int(folds[0])
...
def main() -> None:
    ...
    run_dir = args.run_dir or f"runs/paths_oswt_{args.dataset}_{args.oswt_variant}"
    fundus = bool(defaults["fundus"]) or (
        args.dataset == "generic" and args.fundus_preprocessing
    )
    strength = 0.0 if args.oswt_variant == "identity_v3" else args.oswt_strength
    risk_gate = args.oswt_variant == "shell_warranted"
    ...
    cfg = PathsOSWTConfig(
        ...
        oswt_variant=args.oswt_variant,
        pgf_probes=args.pgf_probes,
        oswt_beta_init=args.oswt_beta_init,
        oswt_tau_init=args.oswt_tau_init,
        oswt_beta_cap=args.oswt_beta_cap,
        oswt_tau_floor=args.oswt_tau_floor,
        oswt_tau_cap=args.oswt_tau_cap,
        oswt_strength=strength,
        ...
        certificate_shortlist_size=args.certificate_shortlist_size,
        epochs=epochs,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        # Generic ORIGIN fields remain part of the serialized identity.  The
        # OSWT trainer replaces its optimizer with one refiner-only group.
        lr=args.paths_refiner_lr,
        head_lr=args.paths_refiner_lr,
        weight_decay=args.weight_decay,
        grad_clip_norm=args.grad_clip_norm,
        encoder_freeze_epochs=epochs,
        scheduler="plateau",
        ...
        amp_unfreeze_scale=args.amp_init_scale,
    ...
    require_one_sha_bound_fold(folds)
    log.info("PATHS-OSWT configuration: %s", asdict(cfg))
    log.info(
    ...
    for fold in folds:
        ...
        manifest = {
            "schema": "paths-oswt-split-v1",
            "dataset": cfg.dataset,
            "fold": fold,
            "signature": signature,
            "evaluation_scope": (
                "outer_test_after_selection"
                if args.include_test
        ...
        model = build_paths_oswt_model(
            ...
            paths_oswt_probe_z=cfg.pgf_probes,
            paths_oswt_beta_init=cfg.oswt_beta_init,
            paths_oswt_tau_init=cfg.oswt_tau_init,
            paths_oswt_beta_cap=cfg.oswt_beta_cap,
            paths_oswt_tau_floor=cfg.oswt_tau_floor,
            paths_oswt_tau_cap=cfg.oswt_tau_cap,
            paths_oswt_strength=cfg.oswt_strength,
            paths_oswt_risk_gate=risk_gate,
        )
        trainer = PathsOSWTTrainer(
            model,
            *loaders,
            cfg,
            fold_dir,
    ...
    log.info("PATHS-OSWT results written to %s", Path(cfg.run_dir) / summary_name)
